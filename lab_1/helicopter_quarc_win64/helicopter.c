 ...
 * C source code generated on : Wed Jan 31 13:46:09 2024
 *
...
static void rate_monotonic_scheduler(void)
{
  ...
  if ((helicopter_M->Timing.TaskCounters.TID[2]) > 4) {/* Sample time: [0.01s, 0.0s] */
    helicopter_M->Timing.TaskCounters.TID[2] = 0;
  }
}
...
void helicopter_output0(void)          /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(helicopter_M)) {
    ...
    if (helicopter_M->Timing.RateInteraction.TID1_2) {
      /* RateTransition: '<S5>/Rate Transition: x' */
      helicopter_B.RateTransitionx = helicopter_DW.RateTransitionx_Buffer0;
    }

    /* End of RateTransition: '<S5>/Rate Transition: x' */

    /* DeadZone: '<S5>/Dead Zone: x' */
    if (helicopter_B.RateTransitionx > helicopter_P.DeadZonex_End) {
      rtb_Backgain = helicopter_B.RateTransitionx - helicopter_P.DeadZonex_End;
    } else if (helicopter_B.RateTransitionx >= helicopter_P.DeadZonex_Start) {
      rtb_Backgain = 0.0;
    } else {
      rtb_Backgain = helicopter_B.RateTransitionx - helicopter_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S5>/Gain: x'
     */
    helicopter_B.Joystick_gain_x = helicopter_P.Gainx_Gain * rtb_Backgain *
      helicopter_P.Joystick_gain_x_Gain;
   ...
   *  Sum: '<S6>/Sum3'
   */
  rtb_Clock = ((helicopter_B.Joystick_gain_x - helicopter_P.Gain1_Gain *
  ...
  if (rtmIsMajorTimeStep(helicopter_M)) {
    if (helicopter_M->Timing.RateInteraction.TID1_2) {
      /* RateTransition: '<S5>/Rate Transition: y' */
      helicopter_B.RateTransitiony = helicopter_DW.RateTransitiony_Buffer0;
    }

    /* DeadZone: '<S5>/Dead Zone: y' */
    if (helicopter_B.RateTransitiony > helicopter_P.DeadZoney_End) {
      rtb_Backgain = helicopter_B.RateTransitiony - helicopter_P.DeadZoney_End;
    } else if (helicopter_B.RateTransitiony >= helicopter_P.DeadZoney_Start) {
      rtb_Backgain = 0.0;
    } else {
      rtb_Backgain = helicopter_B.RateTransitiony - helicopter_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S5>/Gain: y'
     */
    helicopter_B.Joystick_gain_y = helicopter_P.Gainy_Gain * rtb_Backgain *
      helicopter_P.Joystick_gain_y_Gain;
  }
   ...
   *  Gain: '<S2>/Gain1'
   */
  rtb_Derivative = helicopter_B.Joystick_gain_y - helicopter_P.Gain1_Gain *
    helicopter_B.Sum;
...
void helicopter_output2(void)          /* Sample time: [0.01s, 0.0s] */
{
  /* local block i/o variables */
  ...
  /* S-Function Block: helicopter/Joystick/Game Controller (game_controller_block) */
  {
    if (helicopter_P.GameController_Enabled) {
      t_game_controller_states state;
      t_boolean new_data;
      t_error result;
      result = game_controller_poll(helicopter_DW.GameController_Controller,
        &state, &new_data);
      if (result < 0) {
        new_data = false;
      }
  ...
  /* RateTransition: '<S5>/Rate Transition: x' */
  helicopter_DW.RateTransitionx_Buffer0 = rtb_GameController_o4;

  /* RateTransition: '<S5>/Rate Transition: y' */
  helicopter_DW.RateTransitiony_Buffer0 = rtb_GameController_o5;
}

/* Model update function for TID2 */
void helicopter_update2(void)          /* Sample time: [0.01s, 0.0s] */
{
  /* Update absolute time */
...
void helicopter_initialize(void)
{
  ...
  helicopter_B.RateTransitionx = helicopter_P.RateTransitionx_InitialConditio;

  /* Start for RateTransition: '<S5>/Rate Transition: y' */
  helicopter_B.RateTransitiony = helicopter_P.RateTransitiony_InitialConditio;
  ...
  /* S-Function Block: helicopter/Joystick/Game Controller (game_controller_block) */
  {
    if (helicopter_P.GameController_Enabled) {
      ...
      result = game_controller_open(helicopter_P.GameController_ControllerNumber,
        helicopter_P.GameController_BufferSize, deadzone, saturation,
        helicopter_P.GameController_AutoCenter, 0, 1.0,
        &helicopter_DW.GameController_Controller);
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(helicopter_M, _rt_error_message);
  ...
  helicopter_DW.RateTransitionx_Buffer0 =
    helicopter_P.RateTransitionx_InitialConditio;
  ...
  helicopter_DW.RateTransitiony_Buffer0 =
    helicopter_P.RateTransitiony_InitialConditio;
...
void helicopter_terminate(void)
{
  ...
  {
    if (helicopter_P.GameController_Enabled) {
      game_controller_close(helicopter_DW.GameController_Controller);
      helicopter_DW.GameController_Controller = NULL;
    }
  }
}
...
RT_MODEL_helicopter_T *helicopter(void)
{
  ...
  /* Initialize timing info */
  {
    ...
    helicopter_M->Timing.sampleTimes[2] = (0.01);
  ...
  helicopter_M->Timing.stepSize2 = 0.01;

  /* External mode info */
  helicopter_M->Sizes.checksums[0] = (3936747614U);
  helicopter_M->Sizes.checksums[1] = (3606900887U);
  helicopter_M->Sizes.checksums[2] = (748190067U);
  helicopter_M->Sizes.checksums[3] = (2387718013U);
  ...
  {
    helicopter_B.RateTransitionx = 0.0;
    helicopter_B.Joystick_gain_x = 0.0;
    ...
    helicopter_B.RateTransitiony = 0.0;
    helicopter_B.Joystick_gain_y = 0.0;
  ...
  helicopter_DW.RateTransitionx_Buffer0 = 0.0;
  helicopter_DW.RateTransitiony_Buffer0 = 0.0;
